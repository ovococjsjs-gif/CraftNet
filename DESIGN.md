...
- Оплата убывает по ТЗ и **строго не пересекается**:
  завод 195–285 > грузчик 150–190 > повар 110–145 > курьер 25–50.
...
- **Штрафы за срыв**: отмена 25% от оплаты, таймаут 30%; недостающие
  помеченные материалы оцениваются по цене продажи и добавляются к штрафу;
  груз грузчика/курьера конфискуется (+80 за ящик цеха). Лазейка
...
- ✅ **Инжекция зданий починена**: в 1.21.11 elementWeights иммутабелен —
  заменяем список целиком (@Mutable mixin) + дублируем в elements; веса
  pvz/bank 8, factory/cafe 5. Здания появляются в НОВЫХ деревнях.
