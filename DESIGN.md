...
- **Завод** (бригадир, T_FACTORY): сборка редстоун-компонента по схеме — клики по
  ...
  Оплата = 90 + 35 за деталь (+случайные 0..19) → 195..285.
- **Грузчик** (ПВЗ): взять «грузовой ящик» → тащить (Slowness I) к подсвеченному
  жителю. Оплата = clamp(дистанция×4, 150, 190) — всегда между заводом и поваром.
- **Повар** (кафе, T_COOK): сборка по схеме, затем разноска — FOOD_BOX glowing-
  клиентам; сумма заказа (40 + 18 за порцию → 94..140) делится между подачами.
- **Курьер** (ПВЗ): «пакет еды» жителю, 10 + 5 за порцию (3..8 порций) → 25..50.
- Диапазоны **строго не пересекаются**: завод > грузчик > повар > курьер (ТЗ).
...
- ✅ **Казино-апгрейдер** (CasinoState/CasinoManager): ставка — пул в NBT
  (списывается при постановке, «очистить» возвращает), честная формула
  chance = ΣsellPrice_ставки / buyPrice_цели (спред buy/sell играет роль
  house edge при обналичке приза), потолок 1%..95% bp. Ролл серверный,
  мгновенный, запись {spinId,win,bp,target,sicon}; клиентская лента 48 ячеек
  строится детерминированно из spinId и останавливается ровно на решении.
  Вкладки телефона переведены на ряд иконок (до 7 приложений).
