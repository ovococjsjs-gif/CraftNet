...
def build_nbt(layers, entities):
        ...
        for z, row in enumerate(layer):
            for x, ch in enumerate(row):
                ...
                if name not in palette_idx:
                    ...
                    palette.append(name)
                blocks.append((x, y, z, palette_idx[name]))
    ...
    root = {
        "size": L(INT, [sx, sy, sz]),
        "palette": L(COMPOUND, [C({"Name": S(n)}) for n in palette]),
        "blocks": L(COMPOUND, [
            C({"state": I(st), "pos": L(INT, [x, y, z])}) for (x, y, z, st) in blocks
        ]),
...
def pvz():
    ...
    ent = [entity(5, 1, 3, {
        "id": S("minecraft:wandering_trader"),
...
def selfcheck():
    ...
    for fn in sorted(os.listdir(ROOT)):
        ...
        print(f"  {fn}: size={size} blocks={nblocks} entities={nents} DataVersion={root['DataVersion']} OK")
